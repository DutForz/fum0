...
class ChatScreen extends StatefulWidget {
  const ChatScreen({super.key, required this.receiverEmail, required this.receiverId, this.receiverNickname, this.receiverBio, this.receiverAvatarUrl});
  final String receiverEmail;
  final String receiverId;
  final String? receiverNickname;
  final String? receiverBio;
  final String? receiverAvatarUrl;
  String get _displayName { if (receiverNickname != null && receiverNickname!.trim().isNotEmpty) return receiverNickname!.trim(); return receiverEmail; }
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _sendMessage() { final message = _messageController.text; if (message.isEmpty) return; _chatBloc.add(ChatSendMessageRequested(message)); _messageController.clear(); }
      ...
      child: Scaffold(
        ...
        body: Column(children: [
          Expanded(child: BlocBuilder<ChatBloc, ChatState>(builder: (context, state) {
            ...
            return ListView.builder(controller: _scrollController, itemCount: state.messages.length, itemBuilder: (context, index) => _MessageItem(message: state.messages[index], currentUserId: state.currentUserId));
          })),
          Padding(padding: const EdgeInsets.only(bottom: 50), child: Row(children: [
            ...
            IconButton(onPressed: _sendMessage, icon: const Icon(Icons.arrow_upward)),
          ])),
        ]),
      ),
    );
...
class _MessageItem extends StatelessWidget {
  const _MessageItem({required this.message, required this.currentUserId});
