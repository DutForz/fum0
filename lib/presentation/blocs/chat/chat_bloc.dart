...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ChatBloc({required GetMessages getMessages, required SendMessage sendMessage, required ChatRepository chatRepository})
      : _getMessages = getMessages, _sendMessage = sendMessage, _chatRepository = chatRepository, super(const ChatInitial()) {
  ...
  Future<void> _onChatStarted(ChatStarted event, Emitter<ChatState> emit) async {
    ...
    _messagesSubscription = _getMessages(GetMessagesParams(currentUserId: event.currentUserId, otherUserId: event.otherUserId, myPrivateKey: null)).listen((messages) => add(ChatMessagesUpdated(messages)), onError: (Object error) => add(ChatMessagesFailed(error.toString())));
  ...
  Future<void> _initKeys(String currentUserId, String otherUserId) async {
    try {
      ...
      if (encodedRecipientKey != null) _recipientPublicKey = CryptoService.decodeRSAPublicKey(encodedRecipientKey);
      await _messagesSubscription?.cancel();
      _messagesSubscription = _getMessages(GetMessagesParams(currentUserId: currentUserId, otherUserId: otherUserId, myPrivateKey: _myKeyPair?.privateKey)).listen((messages) => add(ChatMessagesUpdated(messages)), onError: (Object error) => add(ChatMessagesFailed(error.toString())));
    } catch (_) {}
  }
