...
abstract class SearchRemoteDataSource { Future<List<UserModel>> searchUsers(String query); }

class SearchRemoteDataSourceImpl implements SearchRemoteDataSource {
  SearchRemoteDataSourceImpl({required FirebaseFirestore firestore}) : _firestore = firestore;
  ...
  Future<List<UserModel>> _searchByField(String field, String query) async {
    final snapshot = await _users.orderBy(field).startAt([query]).endAt(['$query\uf8ff']).get();
    return snapshot.docs.map((doc) => UserModel.fromMap(doc.data())).toList();
  }
  @override Future<List<UserModel>> searchUsers(String query) async {
    final trimmedQuery = query.trim();
    if (trimmedQuery.isEmpty) return const [];
    final seenUids = <String>{};
    final results = <UserModel>[];
    Future<void> addResults(List<UserModel> users) async { for (final user in users) { if (seenUids.add(user.uid)) results.add(user); } }
