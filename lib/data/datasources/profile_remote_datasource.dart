  ...
  Future<UserModel> updateProfile(UserModel profile) async {
    try {
      if (profile.nickname != null && profile.nickname!.trim().isNotEmpty) {
        await _ensureNicknameAvailable(profile.nickname!, profile.uid);
      }

      if (profile.registrationMethod == RegistrationMethod.phone &&
          profile.email.isNotEmpty) {
        final currentUser = _auth.currentUser;
        if (currentUser != null && currentUser.email != profile.email) {
          await currentUser.verifyBeforeUpdateEmail(profile.email);
        }
      }

      final updatedProfile = profile.copyWith(
        nickname: profile.nickname?.trim().isEmpty ?? true
            ? null
            : profile.nickname!.trim(),
        phone: profile.phone?.trim().isEmpty ?? true
            ? null
            : profile.phone!.trim(),
      ...
      await _users.doc(profile.uid).set(
        {
          ...
          'registrationMethod':
              updatedProfile.registrationMethod.toFirestoreValue(),
